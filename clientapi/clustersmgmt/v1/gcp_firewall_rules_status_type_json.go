...
func WriteGcpFirewallRulesStatus(object *GcpFirewallRulesStatus, stream *jsoniter.Stream) {
	...
	if len(object.fieldSet_) > 0 && object.fieldSet_[0] {
		stream.WriteString(GcpFirewallRulesStatusLinkKind)
	} else {
		stream.WriteString(GcpFirewallRulesStatusKind)
	}
	count++
	...
	present_ = len(object.fieldSet_) > 4 && object.fieldSet_[4] && object.rules != nil
	if present_ {
		if count > 0 {
			stream.WriteMore()
		}
...
func ReadGcpFirewallRulesStatus(iterator *jsoniter.Iterator) *GcpFirewallRulesStatus {
	...
	for {
		...
		case "kind":
			...
			if value == GcpFirewallRulesStatusLinkKind {
